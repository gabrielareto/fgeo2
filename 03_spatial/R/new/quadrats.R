...
# 2) All labels with exactly k characters:
...
# How the ID is built
built = "col+row" # c("col+row", "row+col", "sequence")
...
changing.directions = FALSE # maybe not so easy
...
grid.exact$xmax <- pmin(xmax, xmins + quadrat.side.x)
grid.exact$ymax <- pmin(ymax, ymins + quadrat.side.y)
...
# Reorganize and return the output:
out <- grid[,c("entire", "xmin", "ymin", "xmax", "ymax")]
