# <function> <name> quad.to.gxgy </name> <description> Convert quadrat names into x-y coordinates, assuming the first 2 digits
# are the column and the second two the row. Quad is a character.  If the first row and column are 00, set start=0, etc.
...
# </source> </function> <function> <name> rowcol.to.index </name> <description> None given.  </description> <arguments>
...
# </source> </function> <function> <name> gxgy.to.quad </name> <description> Calculate a quadrat name (column number then row
# number, as a 4-digit character string) from gy-gy. If start is set to zero, quadrats start with 0000, otherwise, 0101.
# </description> <arguments> </arguments> <sample> </sample> <source>
gxgy.to.quad <- function(gx, gy, gridsize = 20, plotdim = c(1000, 500), digits = 2, start = "zero") {
    ...
    if (digits != 2) 
        return("Must rewrite if three digit quadrats")
...
# </source> </function> <function> <name> getquadratname </name> <description> Convert x, y coordinates and plot dimensions
# into 4-character quadrat names. If x or y are missing, the quadrat=9999.  </description> <arguments> </arguments> <sample>
# </sample> <source>
...
# </source> </function> <function> <name> convert.rowcol </name> <description> Convert an integer to a character, with a single
# leading zero if the integer is < 10. Does not handle integers >99 </description> <arguments> </arguments> <sample> </sample>
# <source>
...
# </source> </function> <function> <name> gxgy.to.index </name> <description> Assign any location(s) a single index identifying
# the quadrat. The index runs from 1 to the number of quadrats.  </description> <arguments> </arguments> <sample> </sample>
# <source>
...
# </source> </function> <function> <name> index.to.rowcol </name> <description> Calculate the row and column given the quadrat
# index, as calculated in gygy.to.index. Both row and column start at 1, not 0 as in quadrat naming.  </description>
...
# </source> </function> <function> <name> gxgy.to.rowcol </name> <description> Returns row and column for any set of
# coordinates. Rows and columns both start at 1, not 0.  </description> <arguments> </arguments> <sample> </sample> <source>
...
# </source> </function> <function> <name> gxgy.to.hectindex </name> <description> </description> <arguments> </arguments>
# <sample> </sample> <source>
gxgy.to.hectindex <- function(gx, gy, plotdim = c(1000, 500)) {
    if (gx >= plotdim[1] || gy >= plotdim[2] || gx < 0 || gy < 0) 
        return(rep(-1, length(index))) else {
...
# </source> </function> <function> <name> gxgy.to.lxly </name> <description> Given global coordinates and quadrat and plot
# dimensions, calculate local x and y, the within-quadrat coordinates </description> <arguments> </arguments> <sample>
# </sample> <source>
...
# </source> </function> <function> <name> lxly.to.p5 </name> <description> Given local, or within-quadrat, coordinates for a
# 20-m quadrat, return the p5x5; lx and ly must be vectors of equal length. Any values outside [0,20) are returned p5=NA.
# </description> <arguments> </arguments> <sample> </sample> <source>
...
# </source> </function> <function> <name> findborderquads </name> <description> Calculate indices of neighboring quadrats, for
# a given quadrat index.  </description> <arguments> </arguments> <sample> </sample> <source>
...
# </source> </function> <function> <name> create.neighbordata </name> <description> Calculates the mean density in neighboring
# quadrats for every quadrat, given a vector of abundances per quadrat. The vector of abundances must be ordered by quadrat
# index.  </description> <arguments> </arguments> <sample> </sample> <source>
...
# </source> </function> <function> <name> neighbors </name> <description> Finds proportion of neighboring quadrats in which a
# species is present. The input vector is presence-absence for every quadrat. It returns a vector of the same length.
# </description> <arguments> </arguments> <sample> </sample> <source>
...
# dataset, the topographic information of each quadrat comes from a quadrat displaced by slip.horiz and slip.vert units away in
# the original dataset.  </description> <arguments> </arguments> <sample> </sample> <source>
...
# quadrats inside a single larger quadrat.  </description> <arguments> </arguments> <sample> </sample> <source>
...
# <function> <name> distance </name> <description> Calculates the distance from one quadrat to a second quadrat, where quadrats
# are designated by their indices, as created by gxgy.to.index. The two quadrats can be vectors, but must be of the same length
# (or one of the two can be atomic).  Returns a vector of distances same length as input vectors.  </description> <arguments>
# </arguments> <sample> </sample> <source>
distance <- function(quad1, quad2, gridsize = 20, plotdim = c(1000, 500)) {
    ...
    # bad1=pt1$gx<0 | pt1$gy<0 bad2=pt2$gx<0 | pt2$gy<0
    
    # xdist=pt1$gx-pt2$gx ydist=pt1$gy-pt2$gy
    
    # dist=sqrt(xdist^2+ydist^2)
    
    # if(length(pt1)==1 & bad1==T) dist=rep(-1,length(pt2)) else if(length(pt2)==1 & bad2==T) dist=rep(-1,length(pt1))
    
    # dist[bad1]=(-1) dist[bad2]=(-1)
