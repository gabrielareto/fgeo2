...
# <function> <name> growth </name> <description> The principle growth function, constructed like recruitment and mortality. It
# requires two complete datasets, one per census, with dbh, pom, and date for every individual of all species in at least 2
# censuses (see Data Format).  It calculates the mean growth rate in one or more categories defined by the split variables,
# split1 and split2. The column date is required for annualizing rates.  The columns status and stemID are both required, in
# order to determine which stems should have dbh change calculated.  The function trim.growth handles all checks for trees to
# include; excluded are cases where the stemID changes, extreme values based on err.limit and maxgrow, and trees below a
# minimum dbh in the first census. See the description of trim.growth for more information.  Growth requires fill.dimension in
# utilities.r. <br><br> Output of the growth function is a list with components: <ul> <li>rate, the mean annualized growth rate
# per category selected, either dbh increment, or relative growth <li>N, the number of individuals included in the mean (not
# counting any excluded) <li>clim, width of confidence interval; add this number to the mean rate to get upper confidence
# limit, substract to get lower <li>dbhmean, mean dbh in census 1 of individuals included <li>time, mean time interval in years
# <li>date1, mean date included individuals were measured in census 1, as julian object (R displays as date, but treats as
# integer) <li>date2, mean date in census 2 </ul> Pass the list to assemble.demography (in utilities.r) with type='g' to
# convert the list to a data.frame.

# </description> <arguments> <ul> <li> Usually use rounddown=FALSE; if TRUE, all dbh<55 are rounded down to the nearest
# multiple of 5 <li> With method='I', annual dbh increment is calculated, (dbh2-dbh1)/time; with method='E', relative growth
# rate, (log(dbh2)-log(dbh1))/time <li> With stdev=FALSE, confidence limits are returned, otherwise the SD in growth rate per
# group <li> dbhunit must be 'mm' or 'cm' <li> mindbh is the minimum dbh to include in results <li> growthcol defines how
# growth is measured, either 'dbh' or 'agb' (agb=biomass) <li> for err.limit and maxgrow, see trim.growth() <li> split1 and
# split2 must both be vectors of character variables with exactly as many elements as there are rows in the tables census1 and
# census2 (or both can be NULL), for instance, species names, dbh categories, or quadrat numbers </ul> </arguments> <sample>
# CTFSplot('bci',5:6)<br> growth.data=growth(bci.full5,bci.full6)<br> growth.data$rate<br>
# growth.data=growth(bci.full5,bci.full6,split1=bci.full5$sp)<br> growth.data$rate<br> assemble.demography(grow.data,type='g')
# </sample> <source>
growth <- function(census1, census2, rounddown = FALSE, method = "I", stdev = FALSE, dbhunit = "mm", mindbh = 10, growthcol = "dbh", 
    ...
    if (method == "I") 
        growthrate <- (size2 - size1)/time else if (method == "E") 
...
# </source> </function> <function> <name> biomass.growth </name> <description> Like growth(), but calculates change in biomass
# (agb) instead of dbh. The census tables must have a column called agb. There is no trimming done at all -- every tree is
# included, and its entire biomass (the agb column in the standard CTFS data object has total agb, all stems included.)
...
# </source> </function> <function> <name> growth.eachspp </name> <description> This calculates growth for each species in given
# dbh categories. It creates the split variables then uses growth(). Other arguments are as in growth().  </description>
# <arguments> </arguments> <sample> growth.result=growth.eachspp(bci.full5,bci.full6,classbreak=c(10,50,100,300,500)) </sample>
# <source>
growth.eachspp <- function(census1, census2, classbreak = c(10, 100, 300), dbhunit = "mm", mindbh = 10, growthcol = "dbh", err.limit = 4, 
...
# </source> </function> <function> <name> growth.dbh </name> <description> This calculates forest-wide growth in given dbh
# categories. Arguments as for growth().  </description> <arguments> </arguments> <sample>
# growth.dbh=growth.eachspp(bci.full5,bci.full6,classbreak=c(10,50,100,300,500)) </sample> <source>
growth.dbh <- function(census1, census2, classbreak = c(10, 100, 300), dbhunit = "mm", growthcol = "dbh", err.limit = 4, maxgrow = 75, 
...
# </source> </function> <function> <name> growth.indiv </name> <description> This returns a complete table with growth rate of
# every individual, both relative and dbh-increment. The table also includes most other key pieces of information for every
# individual: species, dbh, coordinates. Growth is trimmed with trim.growth, and growth is returned as NA if the individual is
# excluded; note, though, that every individual tree is always included in the table, even if growth=NA. Arguments are the same
# as in growth().  </description> <arguments> </arguments> <sample> growth.table=growth.indiv(bci.full5,bci.full6) </sample>
# <source>
growth.indiv <- function(census1, census2, rounddown = FALSE, mindbh = 10, dbhunit = "mm", err.limit = 4, maxgrow = 75) {
    ...
    growthdata <- data.frame(treeID = census1$treeID, tag = I(census1$tag), sp = I(census1$sp), gx = census1$gx, gy = census1$gy, 
        dbh1 = census1$dbh, dbh2 = census2$dbh, time = time, incgr = incgrowth, expgr = expgrowth, status1 = census1$status, status2 = census2$status)
    
    return(growthdata)
}
# </source> </function> <function> <name> trim.growth </name> <description> This is where growth rates are excluded. It is
# based on a linear model estimating the standard deviation of dbh measures (due to error, that is); the parameters slope and
# intercept define the linear relationship between this error deviation and dbh. Any case where the second dbh measure is more
# than 4 standard deviations below the first is marked false, meaning it will be excluded from growth measurements. The default
# values of slope and intercept are based on dbh remeasure tests done in both 1995 and 2000 at BCI. A line was fitted through
# the absolute dbh errors as a function of dbh in both years; the average slope and intercept is used here. The function also
# excludes any growth rate > 75 mm per yr, cases where the stemID changes, and if the POM changes by more than 5%.  All
# parameters for excluding growth measures based on error can be adjusted: to include all measures, set maxgrow and err.limit
# to very high numbers, such as 10000; to include POM changes, set pomcut to a high number, such as 10; to include cases where
# stemID changed, set exclude.stem.change=FALSE.  This function is usually only used inside the other growth functions.  With
# exclude.stem.change==FALSE, keep all cases where stem changes, regardless of growth (it does not make sense to exclude a
# record based on growth when the growth is based on different stems).  Note that trees are exclude if cens1$dbh<mindbh, but
# not if cens2$dbh<mindbh.  </description> <arguments> </arguments> <sample> </sample> <source>
trim.growth <- function(cens1, cens2, time, slope = 0.006214, intercept = 0.9036, err.limit = 4, maxgrow = 75, pomcut = 0.05, mindbh = 10, 
...
# </source> </function> <function> <name> growth.biomass.indiv </name> <description> Like growth.indiv but based on agb growth,
# not dbh growth. Extreme growth rates (based on dbh growth) are excluded, but cases where the stemID changed are not excluded.
# Here pomcut is used in a very specific way probably only relevant at BCI. If the second pom is higher than the first by more
# than the pomcut, the record is not used. The function trim.growth has already eliminated cases where the stemID is unchanged
# and pom changes, so this will only serve for cases where two different stemIDs have measurements. At BCI, in most cases where
# the second pom is lower than the first and the stem changed, it is a legitimate stem change. But where the second pom is
# higher, it is really the same stem measured at a different pom, and with a different stemID because BCI lacks stem tags.  For
# most plots, especially with stem tags, the default behavior means changes in stem allow changes in pom to be included in
# biomass growth.  </description> <arguments> </arguments> <sample> </sample> <source>
growth.biomass.indiv <- function(census1, census2, mindbh = 10, steminfo = FALSE, dbhunit = "mm", err.limit = 4, maxgrow = 75, 
    rounddown = NULL, exclude.stem.change = TRUE, pomcut = 10000) {
    time <- (census2$date - census1$date)/365.25
    ...
    if (steminfo) 
        growthdata <- data.frame(treeID = census1$treeID, tag = I(census1$tag), stemID = census1$stemID, stemtag = I(census1$StemTag), 
            sp = I(census1$sp), gx = census1$gx, gy = census1$gy, dbh1 = census1$dbh, dbh2 = census2$dbh, agb1 = census1$agb, agb2 = census2$agb, 
            time = time, incgr = incgrowth, expgr = expgrowth, status1 = I(census1$status), status2 = I(census2$status), codes1 = I(census1$codes), 
            codes2 = I(census2$codes)) else growthdata <- data.frame(treeID = census1$treeID, tag = I(census1$tag), sp = I(census1$sp), gx = census1$gx, gy = census1$gy, 
        dbh1 = census1$dbh, dbh2 = census2$dbh, agb1 = census1$agb, agb2 = census2$agb, time = time, incgr = incgrowth, expgr = expgrowth, 
...
DBHtransition <- function(data1, data2, dbhbreaks = c(10, 50, 100, 300, 10000), maxgrow = 45, misscode = "M") {
    ...
    goodrecruit <- trim.growth.mismeasure(dbh1 = rep(10, length(data1$dbh)), dbh2 = data2$dbh, time = (data2$date - data1$date)/365.25, 
        maxgrow = maxgrow)
