...
# Previous evaluation and candidates for removal

An earlier function-by-function evaluation was prepared during the transition from the CTFS scripts to the `ctfs` and `fgeo` packages. The table was edited by Sean McMahon and Gabriel Arellano and discussed with Stuart Davies and Mauro Lepore. The complete record is preserved in [`evaluation/old-evaluation-table.xlsx`](https://github.com/gabrielareto/fgeo2/blob/c053ae2740c942244b222f05893a190faca2187a/evaluation/old-evaluation-table.xlsx).

The table below combines its `decision` and `notes` fields into three practical categories:
...
The candidates are ordered by module and script so the source is easy to locate. Module `01` means `01_utilities/R/new`; module `05` means `05_demography_growth/R/new`.
...
# Due diligence before removal

The due diligence has two narrow goals:
...
if(length(dependent_rows) == 0)
{
  ...
  knitr::kable(dependent_table, row.names = FALSE)
}
...
if(length(dependency_rows) == 0)
{
  ...
  knitr::kable(dependency_table, row.names = FALSE)
}
...
This section will record the present decision for each candidate after reviewing the earlier recommendation, the formal tests, and the due-diligence results above.
