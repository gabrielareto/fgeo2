...
\
Many of the auxiliary functions only describe statistical distributions for random draws during modeling. In R, there exist more general and better maintained alternatives. See `distr`, `extraDistr`, `Runuran`, and `pdqr` packages, among others. These should not be needed if one uses the standard tools for complex modeling (Stan, `brms` package, ...), as these have their own tools for defining the distributions (including things like `brms::custom_family()` for nuanced customization).

An earlier function-by-function evaluation was prepared during the transition from the CTFS scripts to the `ctfs` and `fgeo` packages. That discussion involved Sean McMahon, Stuart Davies, Mauro Lepore and myself (Gabriel Arellano). The results of that discussion were stored in a table edited directly by McMahon and Arellano; see [`evaluation/old-evaluation-table.xlsx`](https://github.com/gabrielareto/fgeo2/blob/c053ae2740c942244b222f05893a190faca2187a/evaluation/old-evaluation-table.xlsx). That review recommended massive removal of all the specialized modeling blocks, and the functions that describe statistical distributions, with caution regarding dependencies somewhere else. I have used also the the [ctfs development notes](https://github.com/forestgeo/ctfs/blob/cc32c7c2a3959ef60b13144f956e99d3a0515322/NEWS.md), [growth tests](https://github.com/forestgeo/ctfs/blob/cc32c7c2a3959ef60b13144f956e99d3a0515322/tests/testthat/test-growth_vs_dbh.R), and [legacy regression tests](https://github.com/forestgeo/ctfs/blob/cc32c7c2a3959ef60b13144f956e99d3a0515322/tests/testthat/test-replace_subset.R). I have complemented that old review with a new review of the code, some tests, evaluation of dependencies, scientific need, and maintained alternatives. The table below is the consolidated evaluation of functions involved in Bayesian, pseudo-Bayesian, or specialized model fitting. Other functions that are also clear candidates for removal will be reviewed somewhere else.
...
removal_functions <- candidate_table$Function[
  candidate_table[["Recommendation"]] == "remove"
]
...
This is a targeted dependency check, not a repository-wide unused-function audit. It can only identify functions connected to the predefined candidate set. Isolated functions and disconnected groups with no path to a candidate require a separate review.
...
By construction, this check cannot identify an isolated function, or a disconnected group of functions, that is not upstream of one of the removal candidates.
...
## Additional functions requiring an explicit decision

The dependency check above identifies the following functions as potentially unnecessary after the obsolete functions are deleted.

-   `AssignDiag()` (`utilities-utilities.r`) replaces the diagonal of a matrix, with special handling for scalar input. Its only current callers are inside `lmerBayes()`. **Decision: remove.**
-   `gsp()` (`utilities-utilities.r`) retrieves an object from a specified search-path position. Its only current caller is `graph.abundmodel()`. **Decision: remove.**
-   `addBinParam()` (`utilities-distributions.r`) creates starting parameters for a growth model with one additional size bin. Its only current caller is `run.growthfit.bin()`. **Decision: remove.**
-   `constant()` (`utilities-distributions.r`) returns the same predicted value for every value of `x`. It is used only as a callback by the obsolete samplers. **Decision: remove.**
-   `linear.model.ctr()` (`utilities-distributions.r`) calculates a straight line after optionally centering `x`. It is used only as a callback by the obsolete growth-bin functions. **Decision: remove.**
-   `split.data()` (`utilities-utilities.r`) splits a data frame into a named list using a column or external grouping vector. Its only current callers are in the obsolete sampler code, but it is also mentioned in an example in `map-map.r` and is the stated inverse of `merge.data()`. **Decision: keep.** It will be reviewed again when reviewing the utilities, and again when reviewing the isolated functions.
-   `dasymnorm()` (`utilities-distributions.r`) calculates the density of a two-piece normal distribution. Its only current callers are in the obsolete abundance model, but it is a general distribution function. **Decision: remove.**
-   `dsymexp()` (`utilities-distributions.r`) calculates the density of a symmetric two-sided exponential distribution. Its only current callers are in the obsolete abundance model; companion cumulative and random-generation functions remain. **Decision: remove.**
-   `dasymexp()` (`utilities-distributions.r`) calculates the density of an asymmetric two-sided exponential distribution. Its only current callers are in the obsolete abundance model; a companion quantile function remains. **Decision: remove.**
-   `dasympower()` (`utilities-distributions.r`) calculates the density of a bilateral power distribution. Its only current callers are in the obsolete abundance model; companion quantile and random-generation functions remain. **Decision: remove.**

Other pending functions can be identified later, from the graph of dependencies, or iterations of the priority-setting routines.
...
\
The updated evaluation suggests removing 72 of the 73 evaluated functions, while keeping `extract.growthdata()` for later review. Other nine additional functions listed above will become useless and are also to be removed. I created some issues for further development inspired by the obsolete code.

# Package structure after initial removals

After removals, this is the structure of fgeo2:

```{r}
#| out-width: "100%"

post_work_commit <- "4f9a619"
post_work_date <- as.Date("2026-09-02")
...
g.after <- graph_from_data_frame(post_edges, directed = TRUE)

subnet <- components(g.after, mode = "weak")
V(g.after)$color <- hcl.colors(subnet$no, "Dark 3")[subnet$membership]
...
The image excludes isolated functions (single functions with no dependents and no dependencies). Different sub-networks have different colors. This graph describes dependency structure; it is not, by itself, an unused-code audit.

# Follow-up: remaining distribution utilities

A later repository-wide pass found 80 top-level function definitions, representing 79 unique names, still active in `01_utilities/R/new/utilities-distributions.r`; one function was defined twice. Only `pospower()` had a production caller outside that script: `individual_grow.table()`. The earlier due diligence did not identify the other functions because it started from the predefined modeling candidates and followed only their dependency paths. Most of the remaining distribution functions were isolated; others formed disconnected internal groups that were unrelated to the candidate set. The post-work graph omitted the isolated functions and did not classify disconnected groups according to external use.

The one live use of `pospower()` is simply a signed power transformation. It can be written directly while preserving the behavior for positive, negative, zero, and missing growth values:
...
After replacing that call, no function in the active `R/new` code, tutorials, or tests uses a definition from `utilities-distributions.r`. The file also mixes elementary wrappers, exploratory calculations, and specialized modeling callbacks. Several functions have clear source-level problems: for example, `fitnorm()` supplies three default parameters while `minum.normal()` reads four, `fit.pdf()` refers to an undefined `par` in its one-parameter branch, and `logistic.ctr()` uses an undefined `inter`. Maintaining or repairing these functions would not support the current focus of fgeo2 on measurements, descriptors, and data preparation.

**Decision:** replace the sole `pospower()` call with the direct signed-power expression and delete `utilities-distributions.r` from `R/new`. The historical implementation remains available in `R/old`, the original CTFS snapshot under `evaluation/Condit`, and Git history.
