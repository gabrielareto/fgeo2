...
Newer `fgeo` code is in a set of four fgeo-related packages: `fgeo.analyze`, `fgeo.plot,` `fgeo.tool`, `fgeo.x`. It depends on the `ctfs` package, which is a forma R package including CFS scripts.

```{r}
# fgeo package is 
#pak::pak("forestgeo/ctfs") # install ctfs
library(ctfs)
library(fgeo)
...
# Quadrat-naming conventions

One of the goals of this package update is to reduce dependency on the "CTFS orthodox format" or other formal assumptions. The quadrat naming or IDing conventions is clearly a point of vulnerability, as the code assumes CCRR convention (with start = 0 or start = 1). It also assumes the BCI convention for 5x5 m subquadrats.

The quadrat IDing convention assumed by old CTFS code is widely adopted but not universal, not even within ForestGEO. Many ForestGEO sites keep multiple versions of the data: one to store within STRI servers and orthodox, and another version they like more and use more often for more internal projects.

I review quadrat-IDing conventions in different datasets that I have used for ForestGEO / AFTS projects.
...
Ideally, the only piece of information really needed is just the global coordinates, {gx, gy}. That is what should be kept in a good, easy-to-use data table.
...
## Quadrat-level information

The best practice would be to have:

-   a tree or stem-level table
-   a quadrat-level table with quadrat-level information; most importantly: location
-   a single unique quadrat-level ID to link both tables

Without any other additional assumption.

The current code deviates significantly from that.

The CTFS code uses two ways of identifying quadrats: quadrat code or name (CCRR convention), and quadrat index (numerical, consecutive). The quadrat index in particular is used often but mostly internally. Users are not familiar with that. It's not used for field work. It's rarely part of the distribution of the dataset. Ideally, we want to get rid of quadrat indexes.

A major limitation of the current code is that quadrat location is treated as *implicit* in quadrat IDing conventions (either CCRR or quadrat index). In other words: one cannot specify {quadrat IDs, locations} without changing their codes or indexes. The only way to make quadrat location explicit is to use `quad.to.gxgy()` or `index.to.gxgy()` but this is not raw data, it simply derives from the assumptions themselves.

Clearly, *arbitrary* quadrat-level IDs should be allowed, to store locations or any other quadrat-level info in quadrat-level tables (maps, etc.). The new could should be able to work under *any* way of identifying quadrats uniquely, regardless of whether that is a numerical code, or alpha-numerical, or whatever.
...
A major improvement would be to allow the user to declare or inform somehow the links between quadrat IDs and quadrat locations, and facilitate the creation of quadrat-level tables with locations and maybe other things.

**In short, a major improvement would be to move from \[rigid quadrat IDs with location implicit\] to \[arbitrary quadrat IDs + explicit locations\].**

In practice, however, people may be storing quadrat-level locations inside those quadrat IDs, so we still have to solve that. The main bottleneck, would be to infer locations from a wide range of quadrat ID conventions. CCRR and consecutive numerical can be useful defaults or guesses, but cannot be enough.
