...
Older CTFS code is within local scripts. We declare paths to the project (watched by git), load two specific versions of the code, and source auxiliary functions.
...
# Exact points before and after the work described in this document:
pre_work_commit  <- "8b1c13366dba84a757fb08ebd9cf5d813c0d1668"
post_work_commit <- "9f3ad4bfc3391ceaf60d2671e2f2e6b3e7e3dfd0"
...
# Evaluate one commit without mixing its functions with another commit.
evaluate_snapshot <- function(path, commit, target_script, modules)
{
  historical_path <- make_dir_to_point_in_history(path, commit)
  on.exit({
    if(dir.exists(historical_path))
    {
      ...
      if(inherits(cleanup, "try-error"))
        warning("Could not remove temporary worktree: ", historical_path)
    }
  }, add = TRUE)

  scripts <- unique(unlist(lapply(modules, function(m) {
    list.files(file.path(historical_path, m, "R/new"),
               full.names = TRUE, pattern = "\\.R$", ignore.case = TRUE)
  })))

  target_file <- scripts[
    tolower(basename(scripts)) == tolower(target_script)
  ]
  if(length(target_file) != 1)
    stop("Could not identify one target script named '", target_script, "'")

  code_env <- new.env(parent = .GlobalEnv)
  for(f in scripts)
    sys.source(f, envir = code_env)

  edges <- get_all_dependencies(envir = code_env)
  edges <- edges[, c("from_function", "to_function")]
  colnames(edges) <- c("from", "to")

  quadrat_functions <- functions_in_script(target_file)
  direct_downstream <- unique(edges$to[
    edges$from %in% quadrat_functions &
      !(edges$to %in% quadrat_functions)
  ])
  ...
  downstream <- setdiff(downstream, quadrat_functions)
  project_dependencies <- unique(edges$from[
    edges$to %in% quadrat_functions &
      !(edges$from %in% quadrat_functions)
  ])
  ...
  code_functions <- code_functions[
    vapply(code_functions, is.function, logical(1))
  ]
  called_functions <- unique(unlist(lapply(code_functions, function(fun) {
    tryCatch(
  ...
  list(
    commit = commit,
    quadrat_functions = quadrat_functions,
    edges = edges,
...
{
  target <- unique(c(snapshot$quadrat_functions, snapshot$downstream))
  keep <- snapshot$edges$from %in% target |
    snapshot$edges$to %in% target
  graph_edges <- snapshot$edges[keep,,drop = FALSE]
  vertex_names <- unique(c(target, graph_edges$from, graph_edges$to))
...
before <- evaluate_snapshot(
  ...
  target_script = "spatial-quadfunc.r",
  modules = modules
)

```

The original evaluation date (2026-05-29) is represented here by the exact commit `8b1c133`. The exact hash makes the result independent of later commits on the same date. Each snapshot is sourced into its own environment, and its temporary Git worktree is removed before `evaluate_snapshot()` returns.
...
quadrat_functions <- before$quadrat_functions
down <- before$downstream
g <- make_dependency_graph(before)
...
In CTFS old code, this seems a relatively well-defined set of functions with surprisingly few dependents and only two direct dependencies on other project functions: `pst()` and `xydistvect()`.
...
Unlike other parts of `fgeo`, the code is not wrapping `ctfs` = CTFS. In fact, the `ctfs` counterpars are not being called at all. Therefore, I have to identify the working code to see if there are differences or something extra in `fgeo` that is not in CTFS original distribution.
...
-   There are two relevant scenarios: either the user keeps quadrat locations explicit (best practice) or implicit (bad practice). Getting and storing quadrat locations permanently is mandatory step for a well-managed project, so it should divide the funtions in two groups: those to get locations vs. those using locations.
...
The sections above record the evaluation that guided the work. This section evaluates the resulting code at commit `9f3ad4b`, after completing the new functions, tutorial, example data, and review of dependent functions.
...
after <- evaluate_snapshot(
  ...
  target_script = "quadrats.R",
  modules = modules
)

count_internal_edges <- function(snapshot)
{
  sum(
    snapshot$edges$from %in% snapshot$quadrat_functions &
      snapshot$edges$to %in% snapshot$quadrat_functions
  )
}

comparison <- data.frame(
  Measure = c(
    "Functions in the quadrat script",
    "Dependencies within the quadrat script",
    "Direct dependents elsewhere in the project",
    "All dependents elsewhere in the project",
    "Dependencies on other project functions"
  ),
  `Before work` = c(
    length(before$quadrat_functions),
    count_internal_edges(before),
    length(before$direct_downstream),
    length(before$downstream),
    length(before$project_dependencies)
  ),
  `After work` = c(
    length(after$quadrat_functions),
    count_internal_edges(after),
    length(after$direct_downstream),
    length(after$downstream),
    length(after$project_dependencies)
  ),
  check.names = FALSE
)

remaining_legacy_calls <- intersect(
  before$quadrat_functions,
  after$called_functions
)

# Stop if the evaluated snapshots no longer give the expected result.
stopifnot(
  identical(
    as.integer(comparison[["Before work"]]),
    c(20L, 18L, 6L, 8L, 2L)
  ),
  identical(
    as.integer(comparison[["After work"]]),
    c(8L, 5L, 1L, 1L, 0L)
  ),
  identical(after$direct_downstream, "allquadratslopes"),
  length(remaining_legacy_calls) == 0
)
...
The quadrat script went from 20 functions to eight. Five form the main interface: `make_quadrats_fast()`, `make_quadrats()`, `get_quadrat_locations_from_outer_coordinates()`, `assign_points_to_quadrats()`, and `compile_points_from_quadrats()`. The other three are short helpers. None of the eight functions depends on a project function outside `quadrats.R`. One helper uses `FNN::get.knn()` explicitly when adjusting local overlaps.

There is one remaining dependent elsewhere in the project: `allquadratslopes()`, through `make_quadrats_fast()`. The scan found no executable calls to any of the 20 removed CTFS quadrat functions.

## Dependencies after work
...
quadrat_functions <- after$quadrat_functions
down <- after$downstream
...
The graph uses the same convention as the initial graph: arrows go from a dependency to the function that uses it; quadrat functions are gold and dependents outside `quadrats.R` are orange. Isolated quadrat functions are shown too.

## Closing status

-   `fullplot.imageJ()`, `allquadratslopes()`, and `rasterize()` were retained and adapted where necessary.
-   `selectrandomquad2()`, `abundanceperquad()`, and `graph.mvnorm()` were removed. Their relevant tasks are covered directly in the tutorial or by standard R operations.
-   `torus.shift()` was removed. A more general treatment of torus translations remains in GitHub issue #3.
-   The tutorial uses the new interface for the main use cases. A permanent lookup table with the 400 real 20 x 20 m Luquillo quadrats is stored in `data_examples/luquillo.quadrats.20x20.csv`.
-   Function-level decisions are recorded in `evaluation/function_inventory.xlsx`.

# Clean up

Each call to `evaluate_snapshot()` removes its temporary Git worktree before returning, including when the evaluation stops with an error. Before creating a worktree, the auxiliary code also prunes registrations left by an interrupted earlier session.
