...
Current CTFS and fgeo code is impossible to adapt to new conventions. The new code needs to be transparent, long, readable, even if repetitive, so it is easy to expand with new conventions, if they arrive. We can start by incorporating some conventions, and we can expand later.
...
-   There is a risky proliferation of functions to convert between pairs of conventions. The complexity of the code in `fgeo` in particular is crazily out of proportion, involving dozens of functions for what are, essentially, very simple calculations. Things must be simplified radically, in terms of number of functions, number of dependencies, and length of code.
-   We cannot avoid the fundamental problem of guessing location from quadrat ID. For this to be general and usable outside ForestGEO's CCRR convention, we need powerful / detailed ways of handling naming or IDing conventions. This would be new complexity. It must be easy to understand and expand.
