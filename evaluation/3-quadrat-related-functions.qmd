...
If the data only contained trees, the only piece of information really needed is just the global coordinates, {gx, gy}. That is what should be kept in a good, easy-to-use data table.

Storing location implicit in a quadrat code is a bad practice. If quadrat location is implicit in quadrat ID, one cannot specify {quadrat IDs, locations} without changing their codes or indexes. This is a great point of vulnerability. A major improvement would be to move from \[rigid quadrat IDs with location implicit\] to \[arbitrary quadrat IDs + explicit locations\].

If there is any type of information obtained at the quadrat or subplot level, then there must be a quadrat or subplot-level table with that information. This is standard data organization for projects that keep data that corresponds to different types of observations. The most important information in such a quadrat-level table is quadrat location. The link with other tables is made as with any other table: through shared keys = quadrat-level ID. Without any extra assumption.

In general, these IDs could take any form. The new could should be able to work under *any* way of identifying quadrats uniquely, regardless of whether that is a numerical code, or alpha-numerical, or whatever. It must not rely on any quadrat ID convention. The step of finding location from ID must be separated, explicit, and rare. Because of that, it should be OK to make it hard for the user: asking for many details, making it interactive, etc. It is not something that should be done again and again or by default within other functions.

To locate quadrats, (xmax, ymax, xmin, ymin) should be is preferred over (x, y), as there could be ambiguity regarding left-bottom corner or centroid or something else. This is closer to polygon language, and easier to make compatible with other ways of dividing plots into pieces.

A general recommendation is: get quadrat-level {xmin, ymin, xmax, ymax} coordinates, store them, use best practices from then on, and you won't need any of the functions we are discussing here at all! You would relate with quadrat-level info the same way you relate with species-level info, habitat-level info, etc. This general recommendation should be clear and strong somewhere in the documentation / tutorials.

## Fieldwork and other situations

The good practices for data storage and linking tables with each other are very clear. And, or course, all the problems are trivial if one already has a data table with global coordinates.
...
The idea of **global sense** is probably important. Perhaps the output of all functions must be sufficient for it to make global sense. (This will be limited to the input, probably, as we do not want to enforce a specific definition of "global" beyond "everything that you included").
...
The task of splitting seems something more "ecological" or useful for analyses than the other routines, which seems closer to compilation of data tables. Splitting the tree data into pieces can be useful for many things: summaries of local communities at different scales, beta-diversity at different scales, etc. There does not seem to be any reason to limit this flexibility at all, if we want users to be able to do very flexible ecological analyses.
...
Naming pieces of a whole may be easy or difficult. The easiest way is to simply use a sequence of numbers. The IDing conventions should me it easy to interpret existing IDs and to generate new IDs.

Splitting, in general, can be treated as arbitrary and temporary quadrats. Their locations don't need to be stored permanently, in general. However, **splitting + IDing convention is *the* way to recover quadrat codes** if the user lost them or if the user keeps quadrat-level information under different IDing conventions. Therefore, the splitting routines must be able to return quadrats neatly organized acoording to any given naming convention, and not just enforce simple numerical membership, for example. In fact, this is the only reason why splitting cannot be trivial in the new code, or simply omitted and substituted by teaching people how to group things into classes or bins.

The split of 20 x 20 m into 5 x 5 m quadrats is just one instance of a general routine to split. It does not require specific functions. We just have to make sure that the IDing standard 11, 12, 13, 13, 21, 22, etc. convention can be passed as an argument. This would cover the `lxly.to.p5()` functionality.

## Quadrat naming or IDing conventions

### Need for generalization?

The conventions to identify quadrats seem central to the tasks that we need to solve. \
\
In general, we want more flexibility than ForestGEO's CCRR. One of the goals of this package update is to reduce dependency on the "CTFS orthodox format" or other formal assumptions. The quadrat IDing convention assumed by old CTFS code is relatively common but not universal, not even within ForestGEO. Many ForestGEO sites keep multiple versions of the data: one to store within STRI servers and orthodox, and another version they like more and use more often for more internal projects.

If different conventions can be expressed in terms of {row, col} then we can re-use the calculations in CTFS. This must apply to any plot that has been divided into a regular grid.

### Review of conventions

This is a review of some possible quadrat or subplot naming or IDing conventions:

-   Sequential numbering: subplots labelled 1, 2, 3... Note these are meaningful numbers, not just arbitrary codes:
    -   Column-major numbering: numbers increase along columns first, then move to the next column.
    -   Row-major numbering: numbers increase along rows first, then move to the next row.
-   CCRR, pasting column (CC) and row (RR) subcodes as in ForestGEO, starting at 0101 or 0000. In general: CR, CCRR, CCCRRR, without separation or with any separator. I know Mpala plot needs more zeros than others.
-   Alternatively: RRCC, etc., with row sub-code first.
-   Alphanumeric grid labels: labels such as A1, A2, B1, B2... or A01, A02, ... or A-01, A-02, etc. There should be no ambiguity about whether the letter refers to the column or to the row.
-   Fine-grid labels: small cells inside a parent quadrat, e.g. 11–44 for a 4 × 4 grid or, starting at 00, e.g. 00–33. But not conceptually different than subplots within plots.
...
-   Regeneration / seedling subplot codes: independent nested labels for smaller-size-class censuses.

Specifically in different ForestGEO datasets \[PENDING\]:

-   BCI:
-   Other STRI plots in Panama, the small ones:
-   Amacayacu:
-   Yasuni:
-   Fushan:
-   Pasoh:
...
-   Korup:
-   Ituri:
-   Mpala: extra zeros! Very long plot
-   Ngel Nyaki:
-   Luquillo:
...
The IDs may have separators or extra characters at the start. For example, to block Excel from messing code. So, in general, we should expect things like:

-   0101
-   01-01
-   \_0101, as Amacayacu has in some tables
-   (01, 01), as Fushan does (they use explicit {gx, gy} coordinates, not just {col, row})
-   etc.

The lack of separators will make Excel remove initial zeros, so zero padding seems something to consider in general, and all the issues and bugs that could be derived from that.

### Nestedness and inheritance

The subquadrats may have codes that only make sense within a given quadrat, or inherit the ID of the mother unit, for example: e.g. 13A, 13-1, 13_1 for pieces of subplot "13", and so on. May refer to things like pieces of a map while digitizing, e.g. Map_0312_1, Map_0312_2, Map_0312_3, etc. Or, perhaps, the mother unit are individual hectares within larger plots.

This might require a `inherit = TRUE / FALSE` parameter or something equivalent? But that may limit the arbitrary application to sub-units within units.

Perhaps it is better to have a couple of simple tutorial on how to combine ID of mother and children units, so the children-level IDs are unique at the higher level too (which is a preferred practice).

Again: if any function returns children-level units, the output must contain: children-level ID, mother-level ID, and (xmin, ymin, xmax, ymax) of the children units. Possibly also mother-level locations too, if code is vectorized-like in terms of output.

### Multiple levels or ways of splitting the plot

Importantly, there can be two (or more) ways of splitting the plot stored in the data. For example T1 / T2 levels in RAINFOR / Forestplots.net. Or the 20 x 20 m vs. 5 x 5 m quadrats in CTFS.

These could be levels or simple ways. We should not assume nestedness.

These different levels may have different naming conventions; e.g. CCRR vs. CR for larger and smaller subquadrats in ForestGEO plots.

This may be relevant for tutorials, more than the code itself. The code should be able to interpret anything as a sum of pieces: plots as sums of subplots, quadrats as sums of subquadrats, etc. One or two or one hundred ways of splitting the plot should not make a big difference, as long as the IDs and features (locations) of those variables are clear.
