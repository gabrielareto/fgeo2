# <function> <name> spparea.sq </name> <description> Function for calculating the number of species in replicated,
# randomly-placed quadrats of various areas. The variable size refers to the dimension (side) of the square, and can be a
# vector; replicates is the number of random draws per dimension. Full plot data are submitted as censdata.  Species are
# counted as the number of unique values of sp in the R Analytical Table; unidentified species are not counted, based on a
# default unidennames; see the function unidentified.species in utilitiesCTFS.r.<br><br> The return value is a list of two
# components. The first is a table giving the mean number (and SD) of individuals and species in all quadrat sizes submitted.
# The second is a table giving the number of individuals and species in every random quadrat created.  In addition, a graph of
...
# plot <li> replicates: the number of random quadrats to create, of each size <li> unidennames: a vector of species names that
# should not be included in species counts (see the function unidentified.species() </ul> </arguments> <sample>
# speciesPerArea=spparea.sq(bci.full6,size=c(10,20,50),mindbh=10,plotdim=c(1000,500),replicates=5,unidennames=c('unid'))
# rowmatch=match(bci.full6$sp,bci.spptable$sp) bci.full6$Genus=bci.spptable$Genus[rowmatch]
...
# </sample> <source>
spparea.sq <- function(censdata, spcolumn = "sp", size, rectdim = 1, mindbh = NULL, plotdim = c(1000, 500), replicates = 10, unidennames = c("**", 
    "UNID", "uniden", "UNIDEN")) {
    ...
    if (is.null(mindbh)) 
        censdata <- subset(censdata, status == "A" & !unidentified.species(spcolumn, exactstr = unidennames)) else censdata <- subset(censdata, status == "A" & dbh >= mindbh & !unidentified.species(spcolumn, exactstr = unidennames))
    ...
    for (i in 1:noquad) {
        ...
        if (i == 1) 
            plot(allsize$area[i], spp[i], pch = 16, ylim = c(1, 1200), xlim = c(0.01, 60), log = "xy") else points(allsize$area[i], spp[i], pch = 16)
    }
    ...
    area[no.area + 1] <- plotdim[1] * plotdim[2]/10000
...
# <function> <name> selectrandomquad </name> <description> Draws rectangular quadrats in a plot at random. Returns a dataframe
# of coordinates defining the corners of all the quadrats.  The variable size is a vector of dimensions; shape allows a
# rectangle to be drawn; rep is the number of replicated random quadrats per dimension submitted. <br><br>
...
# graph the locations of the chosen quadrats on a plot map </arguments> <sample> </sample> <source>
selectrandomquad <- function(size, shape, rep, plotdim = c(1000, 500), graphit = FALSE) {
    ...
    return(data.frame(xlo, ylo, xhi, yhi, area = (allsize.x * allsize.y)/10000))
}
# </source> </function>



# <function> <name> selectrandomquad3 </name> <description> Creates randomly drawn quadrats, using same arguments and producing
# same return value as selectrandomquad, but using a different algorithm.  The lower left x, y coordinates are drawn at random
# from a range extending outside the plot dimensions, then quadrats are used only if all 4 corners fall inside the plot.
...
selectrandomquad3 <- function(size, shape, rep, plotdim = c(1000, 500), graphit = FALSE) {
    ...
    result <- data.frame(xlo, ylo, xhi, yhi)
    if (graphit) 
        graph.quadrats(result, plotdim = plotdim + 2 * size)
    
    result <- data.frame(xlo, ylo, xhi, yhi)[good, ]
    if (graphit) 
        graph.quadrats(result[good, ], clr = "red", add = TRUE)
    
    return(data.frame(xlo, ylo, xhi, yhi, area = (allsize.x * allsize.y)/10000)[good, ])
}
# </source> </function>
...
graph.quadrats <- function(coord, plotdim = c(1000, 500), clr = "black", add = FALSE) {
    ...
    if (!add) 
        plot(xlo, ylo, xlim = c(0, plotdim[1]), ylim = c(0, plotdim[2]), pch = 16, cex = 0.5, col = "white")
...
# <function> <name> coverage.diag </name> <description> Draws a diagonal across a plot, from lower left to upper right (if
# slope==1), upper left to lower right (if slope==-1), or straight across the middle (if slope==0).  and determines for every
# point along the diagonal what fraction of a series of quadrats it is inside.  The quadrats are defined by their four corners
# in randomquads: xlo, ylo, xhi, yhi.<br><br> This is only used in testing how well random quadrat draws include corners and
# edges of a plot.  </description> <arguments> </arguments> <sample> </sample> <source>
coverage.diag <- function(randomquads, slope = 1, plotdim = c(1000, 500), graphit = FALSE) {
    x <- 0:plotdim[1]
    if (slope == 1) 
        y <- x * (plotdim[2]/plotdim[1]) else if (slope == (-1)) 
        y <- plotdim[2] - x * (plotdim[2]/plotdim[1]) else if (slope == 0) 
        y <- rep(plotdim[2]/2, length(x)) else return("Enter slope of 1, 0, of -1\n")
    
    pts <- data.frame(x, y)
    
    covered <- apply(pts, 1, ispt.inside, coord = randomquads)
    if (graphit) 
        plot(x, covered, type = "l")
