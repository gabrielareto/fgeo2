# <function> <name> abundance </name> <description> Calculates total abundance or basal area, dividing data with 1 or 2
# categorical variables.  The categorical variables must be submitted as vectors whose length matches exactly as the number of
# rows in the plot data submitted (so one per tree or stem).  The first vector should be the one with the most categories (for
# instances, split1=species, split2=dbhcategory). <br><br> The mean measurement date for all individuals in each category is
# also calculated. <br><br>

# For abundance, set type='abund' (the default), for basal area, set type='ba'. Be sure to use the R Analytical Stem Table for
# basal area. For abundance, either the stem table or full table can be used, the former to count stems, the latter
# trees.<br><br>

# The object returned is a list, the first element named either abund or ba, the second named meandate. Each is an array of one
# or two dimensions, depending on how many split variables were submitted: each of the dimensions of the array handles one of
# the categorical variables.  </description> <arguments> <ul> <li> censdata: an R Analytical Table for a full plot census,
# either full or stem <li> type: must be either 'abund', 'ba', or 'agb' <li> alivecode: all codes defining alive; the default
# 'A' is the standard CTFS designation for living trees or stems <li> mindbh: the minimum diameter above which the counts are
# done; if NULL, all (living) are included <li> dbhunit: 'cm' or 'mm', only used for basal area <li> split1: a vector of
# categories, one per individual <li> split2: another vector of categories, one per individual </ul> </arguments> <sample>
# CTFSplot('bci',5:6,'full')<br> CTFSplot('bci',5:6,'stem')<br> total=abundance(bci.full5,mindbh=10)<br> total$abund<br>
# total$meandate<br> totalstem=abundance(bci.stem5,mindbh=10)<br>
# BAperSpecies=abundance(bci.stem5,type='ba',mindbh=10,split1=bci.stem5$sp)<br> head(BAperSpecies$ba)<br>
# head(BAperSpecies$meandate) </sample> <source>
abundance <- function(censdata, type = "abund", alivecode = c("A"), mindbh = NULL, dbhunit = "mm", split1 = NULL, split2 = NULL) {
    ...
    if (!is.null(mindbh)) 
        inc <- censdata$dbh >= mindbh else inc <- rep(TRUE, length(censdata$dbh))
    ...
    if (type == "abund") 
        abund <- tapply(censdata$dbh[alive & inc], groupvar, length) else if (type == "ba") 
        abund <- tapply(censdata$dbh[alive & inc], groupvar, basum, mindbh = mindbh, dbhunit = dbhunit) else if (type == "agb") 
        abund <- tapply(censdata$agb[alive & inc], groupvar, sum, na.rm = TRUE)
    # browser()
    ...
    if (type == "ba") 
        names(result)[1] <- "ba" else if (type == "agb") 
        names(result)[1] <- "agb"
...
# last category will be >= the final division </ul> </arguments> <sample> </sample> <source>
...
# <function> <name> pop.change </name> <description> Finds abundance, basal area, or agb in two censuses and the rate of change
# between them.  Accepts two dataframes, each an R Analytical Table for one census, the earlier census first. <br><br>

# Do not use this function with diameter categories as a split variable! The results won't make sense.  The categories need to
# be permanent attributes, such as species, genus, quadrat. Use instead pop.change.dbh to find population change of dbh
# categories. <br><br>

# Mean census date for a species is not the mean census date for all living individuals in that census, but the mean census
...
# the date on which the quadrat they later appear in was censused in the first census. Plants dead in the second census get a
# census date equal to the date on which their quadrat was censused <br><br>

# The return value is a list of 6 components: <ul> <li> N.1 (or BA.1 or AGB.1): an array of abundance (or basal area or agb) in
# the first census; one dimension of the array for split1, the second for split2 <li> N.2 (or BA.2 or AGB.2): abundance (or
# basal area or agb) in the second census in a matching array <li> date1: mean date of first census in a matching array <li>
# date2: mean date of second census in a matching array <li> interval: the time interval in years in a matching array <li>
# little.r: the rate of population change in a matching array, (log(N2)-log(N1))/time </ul> This list can be submitted to
# <i>assemble.demography</i> (topic utilitiesCTFS) to convert into a convenient table.  </description> <arguments> See
# abundance() </arguments> <sample> bcichange=pop.change(bci.full5,bci.full6,type='abund',split1=bci.full5$sp,mindbh=10)<br>
# str(bcichange)<br> head(bcichange$N.1)<br> change.table=assemble.demography(bcichange,type='a')<br> head(change.table)
# </sample> <source>
pop.change <- function(census1, census2, type = "abund", dbhunit = "mm", alivecode = c("A"), mindbh = NULL, split1 = NULL, split2 = NULL) {
    ...
    if (type == "ba") 
        names(result)[1:2] <- paste("BA", 1:2, sep = ".") else if (type == "agb") 
        names(result)[1:2] <- paste("AGB", 1:2, sep = ".")
    
    return(result)
}
# </source> </function> <function> <name> pop.change.dbh </name> <description> Finds abundance or basal area in two censuses
# and the rate of change between them, in several dbh categories.  Accepts two dataframes, each an R Analytical Table for one
# census, the earlier census first.  Only one additional splitting variable (other than dbh category) is allowed. Typically,
# this is species, but genus or quadrat are other examples.<br><br> The return value is a list of two elements, one name abund
# (or ba) and the other meandate, just as other abundance results.  Each is a table having one pair of columns for every dbh
# category: the first for census 1, the second for census 2. So if there are 3 dbh categories, the table has 6 columns. The
# rows of the table are the splitting variable (eg, species).  </description> <arguments> See abundance() </arguments> <sample>
# Nchange=pop.change.dbh(bci.full5,bci.full6,classbreak=c(10,100,300))<br> Nchange$abund<br>
# BAchangePerSpp=pop.change.dbh(bci.full5,bci.full6,classbreak=c(10,100),split=bci.full5$sp)<br> head(BAchangePerSpp$ba)
# </sample> <source>
pop.change.dbh <- function(census1, census2, type = "abund", dbhunit = "mm", alivecode = c("A"), classbreak = c(10, 100, 300), 
    split = NULL) {
    if (is.null(split)) 
        split <- rep("all", dim(census1)[1])
    ...
    if (!is.null(split)) 
        cols <- as.vector(t(outer(classbreak, 1:2, paste, sep = "_"))) else cols <- paste("all", 1:2, sep = "_")
...
# <function> <name> ba </name> <description> Calculates the individual basal areas (in square meters) for all submitted dbhs.
# The dbh units must be submitted, either 'cm' or 'millimeters'. The return value is a vector of basal area values of same
# length as the submitted vector of dbhs.  </description> <arguments> </arguments> <sample> </sample> <source>
...
# <function> <name> basum </name> <description> Returns the basal area summed over all submitted dbhs. NAs can be included, as
# sum will be completed with na.rm=TRUE.  </description> <arguments> </arguments> <sample> </sample> <source>
...
# <function> <name> abund.manycensus </name> <description> Collect abundances of all species across several censuses. The full
# R census tables are submitted as a list, as many as desired. The argument type can be used to choose basal area or agb, or
# the default for number of individuals. The mindbh to include must be given as an argument, but it can be NULL. If the latter,
# trees are counted if they have no dbh, as long as status=A. By default, any tree ever given code M is not counted in any
# census, but set excludestatus=NULL to include them.<br><br> A character vector of species codes can be submitted as
# excludespp, for instance those for unidentified trees.  </description> <arguments> </arguments> <sample>
...
# head(N)<br> colSums(N)<br> apply(N,2,countspp)<br>
# N=abund.manycensus(allcns=list(bci.full5,bci.full6),mindbh=10,type='abund',excludespp=c('uniden','tremxx'),excludestatus=NULL)<br>
# </sample> <source>
abund.manycensus <- function(allcns = list(bci.full1, bci.full2, bci.full3), mindbh, dbhunit = "mm", type = "abund", excludespp = NULL, 
    excludestatus = NULL) {
    nocns <- length(allcns)
    exclude <- numeric()
    if (!is.null(excludestatus)) {
        ...
        if (length(exclude) > 0) 
            for (j in 1:nocns) allcns[[j]] <- allcns[[j]][-exclude, ]
    }
    
    if (type == "abund") 
        innertype <- "a" else innertype <- type
